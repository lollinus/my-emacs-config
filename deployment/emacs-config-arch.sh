#!/usr/bin/env bash
# emacs-config-arch.sh — Build and install Emacs from source on Arch Linux / EndeavourOS
#
# Usage:
#   ./emacs-config-arch.sh                   # Emacs 30.2 tarball (stable, recommended)
#   ./emacs-config-arch.sh --git             # git master (bleeding-edge)
#   ./emacs-config-arch.sh --ref master      # explicit branch
#   ./emacs-config-arch.sh --ref emacs-30    # release branch
#   ./emacs-config-arch.sh --ref emacs-30.2  # release tag
#   ./emacs-config-arch.sh --ref a1b2c3d4    # specific commit SHA
#
# The binary is installed to ~/.local/. No sudo required for the install step.
# Prerequisites (run once):
#   sudo pacman -S --needed $(grep -v '^#' emacs-pkgs-arch.txt | tr '\n' ' ')
...
check_deps() {
  ...
  local missing=()
  for pkg in gcc tree-sitter gtk3 harfbuzz; do
...
package_emacs() {
  ...
  local out="${BUILD_DIR}/${label}-arch.tar.xz"
