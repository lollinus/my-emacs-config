...
SCOPE is the scope of the search, such as 'project or 'subdirs." nil nil)

(autoload (quote cedet-cscope-expand-filename) "cedet-cscope" "\
Expand the FILENAME with CScope.
Return a fully qualified filename." t nil)
...
Test out some file name conversions." t nil)
...
Show the root of a GNU Global area under the current buffer." t nil)
...
`default-directory' is used." nil nil)
...
Return a filename relative to the default directory." t nil)
...
of just logging the error." t nil)

(autoload (quote cedet-utest-batch) "cedet-utests" "\
Run the CEDET unit tests in BATCH mode." nil nil)
...
Insert the contents of HASH-TABLE inserting PREFIX before each element." nil nil)
...
If PARENT is non-nil, it is somehow related as a parent to thing." nil nil)

(autoload (quote data-debug-mode) "data-debug" "\
Major-mode for the Analyzer debugger.

\\{data-debug-map}" t nil)

(autoload (quote data-debug-new-buffer) "data-debug" "\
Create a new ddebug buffer with NAME." nil nil)

(autoload (quote data-debug-edebug-expr) "data-debug" "\
Dump out the contets of some expression EXPR in edebug with ddebug." t nil)
...
If the result is a list or vector, then use the data debugger to display it." t nil)
...
Only checks one based on which kind of Emacs is being run." nil nil)
...
PROMPT, INITIAL, HIST, and DEFAULT are the same as for `completing-read'." nil nil)
...
Be sure to call `pulse-reset-face' after calling pulse." nil nil)

(autoload (quote pulse-test) "pulse" "\
Test the lightening function for pulsing a line.
When optional NO-ERROR Don't throw an error if we can't run tests." t nil)

(autoload (quote pulse-momentary-highlight-overlay) "pulse" "\
Pulse the overlay O, unhighlighting before next command.
Optional argument FACE specifies the fact to do the highlighting." nil nil)
