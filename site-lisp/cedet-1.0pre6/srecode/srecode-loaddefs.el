...
do not contain any text from preceeding or following text." nil nil)

(autoload (quote srecode-semantic-handle-:indent) "srecode-args" "\
Add macros into the dictionary DICT for indentation." nil nil)

(autoload (quote srecode-semantic-handle-:region) "srecode-args" "\
Add macros into the dictionary DICT based on the current :region." nil nil)

(autoload (quote srecode-semantic-handle-:user) "srecode-args" "\
Add macros into the dictionary DICT based on the current :user." nil nil)

(autoload (quote srecode-semantic-handle-:time) "srecode-args" "\
Add macros into the dictionary DICT based on the current :time." nil nil)

(autoload (quote srecode-semantic-handle-:file) "srecode-args" "\
Add macros into the dictionary DICT based on the current :file." nil nil)

(autoload (quote srecode-semantic-handle-:system) "srecode-args" "\
Add macros into the dictionary DICT based on the current :system." nil nil)

(autoload (quote srecode-semantic-handle-:kill) "srecode-args" "\
Add macros into the dictionary DICT based on the kill ring." nil nil)
...
Compile the templates from the file FNAME." nil nil)
...
HEADER - Shown section if in a header file." nil nil)
...
assocated with a buffer or parent." nil nil)

(autoload (quote srecode-adebug-dictionary) "srecode-dictionary" "\
Run data-debug on this mode's dictionary." t nil)

(autoload (quote srecode-dictionary-dump) "srecode-dictionary" "\
Dump a typical fabricated dictionary." t nil)
...
It is assumed that the comment occurs just in front of FCN-IN." t nil)
...
It is assumed that the comment occurs just after VAR-IN." t nil)
...
Test the srecode field manager." t nil)
...
Prefix each line of STR with the comment prefix characters." nil nil)
...
Optional argument MODE specifies the mode table to use." nil nil)
...
all template files for that application will be loaded." nil nil)
...
tables that do not belong to an application will be searched." nil nil)
...
DEFAULT is what to use if the user presses RET." nil nil)
...
Insert the previously inserted template (by name) again." t nil)

(autoload (quote srecode-insert) "srecode-insert" "\
Inesrt the template TEMPLATE-NAME into the current buffer at point.
DICT-ENTRIES are additional dictionary values to add." t nil)

(autoload (quote srecode-insert-fcn) "srecode-insert" "\
Insert TEMPLATE using DICTIONARY into STREAM." nil nil)
...
FILENAME_AS_CLASS - file converted to a Java class name." nil nil)
...
Optional argument RESET forces a reset of the current map." t nil)

(autoload (quote srecode-adebug-maps) "srecode-map" "\
Run ADEBUG on the output of `srecode-get-maps'." t nil)
...
Add macroes into the dictionary DICT based on the current :tag." nil nil)
...
Assumes the cursor is in a tag of class type.  If not, throw an error." nil nil)
...
variable default values, and other things." nil nil)

(autoload (quote srecode-semantic-apply-tag-to-dict-default) "srecode-semantic" "\
Insert features of TAGOBJ into dictionary DICT." nil nil)
...
as `function' will leave point where code might be inserted." nil nil)
...
HIST is the history value, otherwise `srecode-read-variable-name-history'
     is used.
DEFAULT is the default if RET is hit." nil nil)
...
MODE - The mode of this buffer.  If not declared yet, guess." nil nil)
...
Major-mode for writing srecode macros." t nil)
...
Test various template insertion options." t nil)
...
Argument NEWNODE is the name of the new node." t nil)
...
Adds the following:
  ...
  NEXTLEVEL - One below level" nil nil)
...
Adds the following:
  TAGDOC - Texinfo formatted doc string for :tag." nil nil)
